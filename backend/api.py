...
def question1():
    ...
    query = """MATCH (a:User {screen_name: $screen_name})-[:Post]->(t_a:Tweet {type:'tweet'})<-[:Reply]-(t_b:Tweet {type:'reply'})<-[:Post]-(b:User) \
               RETURN DISTINCT a.screen_name, a.id, t_a.text, t_a.id, t_a.timestamp, COLLECT([b.screen_name, b.id, t_b.text, t_b.id, t_b.timestamp]) \
               ORDER BY t_a.timestamp DESC, t_b.timestamp DESC"""

    # t_b is a "reply tweet" that reply to one kind of tweet posted by "a".
    # t_b.id is a id of t_b  tweet itself
    # b.id is a id of the user who tweet t_b
    ...
    for i in range(len(resp[0])):
        tweet_tmp = []
        for j in range(len(resp[0][i])):
            if j != 5:
                tweet_tmp.append(str(resp[0][i][j][1]))
            elif j == 5:
                reply_tmp = []
                for k in range(len(resp[0][i][j][1][0][1])):
                    reply_tmp.append(resp[0][i][j][1][0][1][k][1])
                total_reply.append(reply_tmp)
        total_tweet.append(tweet_tmp)
        res.append(tweet_tmp)
        res.append(reply_tmp)

    # print(len(total_reply))
    # print(len(total_tweet))
    res = total_tweet + total_reply
    # return jsonify(resp)
    return jsonify(res)
...
def question2():
    ...
    print("##########")
    print(resp[1][0][0][1])
    print(type(resp[1][0][0][1]))
    clean_res = {"result": resp[1][0][0][1]}
...
def question5():
    ...
    for result in results:
        ...
        resp.append(
            [[a_id, a_screen_name], [b_id, b_screen_name], [c_id, c_screen_name]]
        )
